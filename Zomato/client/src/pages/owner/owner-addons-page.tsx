...
export const OwnerAddonsPage = () => {
  ...
  const [editingAddon, setEditingAddon] = useState<OwnerAddon | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<OwnerAddon | null>(null);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      toast.error(getApiErrorMessage(error, "Unable to load your addons."));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const openEditModal = (addon: OwnerAddon) => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!form.restaurantId || !form.parentId) {
      toast.error("Choose one of your restaurants and assign the addon to a dish or combo.");
      return;
    }
    ...
    try {
      ...
      if (editingAddon) {
        await updateOwnerAddon(editingAddon.id, payload);
        toast.success("Addon updated successfully.");
      } else {
        await createOwnerAddon(payload);
  ...
  const handleDeleteAddon = async () => {
    ...
    setIsDeleting(true);
    try {
      await deleteOwnerAddon(deleteTarget.id);
  ...
  return (
    <div className="space-y-8">
      <SectionHeading
        eyebrow="Owner addons"
        title="Extras and upgrades for your menu."
        description="Control your own dish and combo addons, including drinks, dips, sides, upgrades, and dessert upsells."
        action={
      ...
      {isLoading ? (
        ...
        <>
          <AdminDataTable
            ...
            emptyDescription="Create your first addon to unlock richer upsells on dishes and combos."
            columns={[
              {
                key: "addon",
                label: "Addon",
      ...
      <ConfirmDangerModal
        ...
        title="Delete addon"
        description="This permanently removes the addon from the assigned dish or combo."
